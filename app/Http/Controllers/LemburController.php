...
class LemburController extends Controller
{
    public function index()
    {
        ...
        $lembur = Lembur::where('karyawan_id', $user_login)->where('tanggal', $tglkmrn)->get();
        if($lembur->count() > 0) {
            foreach($lembur as $l) {
        ...
        if (auth()->user()->is_admin == 'admin') {
            return view('lembur.index', [
                'title' => 'Absen Lembur',
                'lembur' => Lembur::where('karyawan_id', $user_login)->where('tanggal', $tanggal)->get()
            ]);
        } else {
            return view('lembur.indexuser', [
                'title' => 'Absen Lembur',
                'lembur' => Lembur::where('karyawan_id', $user_login)->where('tanggal', $tanggal)->first()
            ]);
        }
