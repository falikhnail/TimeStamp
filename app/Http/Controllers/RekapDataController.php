...
class RekapDataController extends Controller
{
    ...
    //     $user = Karyawan::orderBy('name', 'ASC')->paginate(10)->withQueryString();
    ...
    public function getData()
    {
        ...
        // Tentukan query untuk mengambil data karyawan
        $query = Karyawan::query();

        // Jika user adalah admin_divisi, filter berdasarkan divisi
        ...
        $data_user = $query->orderBy('name', 'ASC')->paginate(10)->withQueryString();
    ...
    {
        $cek = Payroll::where('karyawan_id', $request['karyawan_id'])->where('bulan', $request['bulan'])->where('tahun', $request['tahun'])->first();
        if ($cek) {
            ...
            $user = Karyawan::find($request['karyawan_id']);
