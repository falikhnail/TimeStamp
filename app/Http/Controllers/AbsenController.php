    ...
    public function dataAbsen(Request $request)
{
    ...
    // Ambil parameter dari request untuk filter pencarian
    $userId = $request->input('user_id');
    $mulai = $request->input('mulai') ? Carbon::parse($request->input('mulai')) : null;
    $akhir = $request->input('akhir') ? Carbon::parse($request->input('akhir')) : null;

    // Query dasar untuk absensi
    $query = MappingShift::query();
    ...
    // Filter berdasarkan user_id jika dipilih
    if ($userId) {
        $query->whereHas('karyawan', function ($q) use ($userId) {
            $q->where('id', $userId);
        });
    }

    // Cek peran pengguna dan sesuaikan kueri
    if ($user->is_admin === 'admin') {
        // Jika pengguna adalah admin, gunakan query yang sudah dibentuk
        $data_absen = $query->paginate(10)->withQueryString();
    } elseif ($user->is_admin === 'admin_divisi') {
        // Jika pengguna adalah admin_divisi, ambil jabatan pengguna
        $divisi = $user->jabatan->nama_jabatan ?? '';

        // Tambahkan filter jabatan untuk admin divisi
        $data_absen = $query->whereHas('karyawan', function ($q) use ($divisi) {
            $q->whereHas('jabatan', function ($q) use ($divisi) {
                $q->where('nama_jabatan', $divisi);
            });
        })->paginate(10)->withQueryString();
    } else {
        // Tangani kasus ketika pengguna tidak memiliki peran yang sesuai atau tidak terotorisasi
        $data_absen = collect(); // Mengembalikan koleksi kosong atau tangani sesuai kebutuhan
    }

    // Ambil data pengguna; pertimbangkan untuk menggunakan cache jika diperlukan
    $users = Karyawan::select('id', 'name')->get();
    ...
    public function maps($lat, $long, $userid)
    {
        ...
        if (auth()->user()->is_admin == 'admin') {
            return view('absen.maps', [
                ...
                'lat' => $lat,
                'long' => $long,
                'data_user' => Karyawan::findOrFail($userid)
            ]);
        } else {
    ...
    public function editMasuk($id)
    {
        ...
        $user = Karyawan::findOrFail($mapping_shift->karyawan_id);
        $lokasi = $user->Lokasi;
    ...
    public function prosesEditMasuk(Request $request, $id)
    {
        ...
        $user = Karyawan::findOrFail($user_id);
    ...
    public function pengajuanProses(Request $request, $id)
    {
        ...
        $user = Karyawan::find($jabatan->manager);
        ...
        $user1->notify(new \App\Notifications\UserNotification);
