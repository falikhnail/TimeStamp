...
class RekapExport implements FromQuery, WithColumnFormatting, WithMapping, WithHeadings,ShouldAutoSize,WithStyles
{
    ...
    public function headings(): array
    {
        return [
            ...
            'Total Libur',
            'Total Telat',
            'Total Izin Meninggalkan Pekerjaan',
            'Total Pulang Cepat',
            'Total Lembur',
    ...
    public function map($model): array
    {
        ...
        $sakit = $model->MappingShift->whereBetween('tanggal', [$tanggal_mulai, $tanggal_akhir])->where('status_absen', 'Izin Sakit')->count();
        $izin_masuk = $model->MappingShift->whereBetween('tanggal', [$tanggal_mulai, $tanggal_akhir])->where('status_absen', 'Izin Masuk')->count();
        $izin_telat = $model->Cuti()->whereBetween('tanggal', [$tanggal_mulai, $tanggal_akhir])->where('nama_cuti', 'Izin Telat')->count();
        ...
        $izin_meninggalkan_pekerjaan = $model->Cuti()
        ->where('nama_cuti', 'Izin Meninggalkan Pekerjaan')
        ->whereBetween('tanggal', [$tanggal_mulai, $tanggal_akhir])
        ->get();

        $total_jam_izin_meninggalkan = 0;

        foreach ($izin_meninggalkan_pekerjaan as $izin) {
            $jam_awal = new \DateTime($izin->jam_awal);
            $jam_akhir = new \DateTime($izin->jam_akhir);
            $selisih = $jam_awal->diff($jam_akhir);
            $total_jam_izin_meninggalkan += $selisih->h + ($selisih->i / 60); // Total jam dan menit
        }

        $jam_total = floor($total_jam_izin_meninggalkan); // Ambil bagian jam
        $menit_total = ($total_jam_izin_meninggalkan - $jam_total) * 60; 
        ...
        return [
            ...
            $jam_total . " Jam " . round($menit_total) . " Menit",
            //  count($izin_meninggalkan_pekerjaan) . ' x'
            // . $jumlah_telat . " x",
    ...
    {
        return Karyawan::orderBy('name', 'ASC');
    }
}
